...
export default (req: Request, res: Response) => {
    res.locals = {
        ...
        getPath: ({ isMenu, path }: { isMenu: boolean; path: string }) => {
            return isMenu ? `/user${path}` : "javascript:;";
        },

        getBreadcrumb: (menu: Array<{ path: string; menuName: string; children?: Array<{ path: string; menuName: string }> }>, current: string) => {
            if (current === "/") return "";
            ...
            menu.forEach((item) => {
                if (item.path === currentPath1) {
                    if (item.children) {
                        res += `<a href="/user${item.path}">${item.menuName}</a>`;
